...
export default function Home() {
  ...
  const expertiseItems = parseJSON(siteConfig?.expertiseJSON, [
  ...
  const experienceItems = parseJSON(siteConfig?.experienceJSON, [
    { year: "2023 — Present", role: "Freelance Full-Stack Developer", desc: "Building custom web applications, delivering intuitive UIs, and robust backends for various clients. Focusing on scalable architectures and AI integrations." },
    { year: "2022 — 2023", role: "Data Science Intern", desc: "Developed predictive models and data visualization dashboards. Assisted in data cleaning, exploratory data analysis, and deploying ML models to production." }
  ]);

  const educationItems = parseJSON(siteConfig?.educationJSON, [
    { year: "2021 — 2025", role: "BS in Data Science and Applications", desc: "Comprehensive study of Mathematics, Statistics, Machine Learning, and Software Development. Consistently maintaining a strong GPA." }
  ]);

  const testimonialsItems = parseJSON(siteConfig?.testimonialsJSON, []);
          ...
          <div className="flex flex-col xl:flex-row xl:items-start justify-between gap-12 xl:gap-8 mb-8">
            ...
            <div className="max-w-[700px] relative z-10 w-full xl:w-2/3">
              <div className="font-mono text-[11px] tracking-[0.12em] text-theme-accent uppercase mb-5 flex items-center gap-2 fade-up">
                ...
                {siteConfig?.heroEyebrow || "AVAILABLE FOR OPPORTUNITIES"}
              </div>

              <div className="flex gap-1.5 flex-wrap mb-7 fade-up delay-75">
                {(siteConfig?.heroTags ? siteConfig.heroTags.split(',') : [
                  "IIT Madras'28", "INSPIRE SHE Scholar", "BSc Mathematics", "Open to Internships"
                ]).map((tag: string) => (
              ...
              <h1 className="font-serif text-[clamp(44px,6vw,76px)] leading-[1.08] tracking-[-0.02em] text-theme-text mb-2 fade-up delay-150 relative">
                <div className="flex flex-col">
                  <span className="block text-theme-text">
                    {siteConfig?.heroTitle ? siteConfig.heroTitle.split('\n')[0] : "Sahil Bind —"}
                  </span>
                  <span className="flex flex-col md:flex-row md:items-baseline md:gap-3">
                    <em className="italic text-theme-accent">
                      {siteConfig?.heroTitleHighlight || "Data Scientist"}
                    </em>
                    <span className="text-theme-text2">
                      {siteConfig?.heroTitle ? siteConfig.heroTitle.split('\n').slice(1).join(' ') : "& Developer."}
                    </span>
                  </span>
                </div>
              </h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-5">
            {loadingProjects ? (
              Array.from({ length: 3 }).map((_, i) => (
                <div key={i} className="bg-theme-card border border-theme-border rounded-3xl overflow-hidden animate-pulse">
                <div className="h-40 bg-theme-bg2" />
                  <div className="p-5 space-y-3">
                    <div className="h-3 w-20 bg-theme-bg2 rounded" />
                    <div className="h-5 w-2/3 bg-theme-bg2 rounded" />
                    <div className="h-3 w-full bg-theme-bg2 rounded" />
                    <div className="flex gap-1.5"><div className="h-4 w-12 bg-theme-bg2 rounded" /><div className="h-4 w-14 bg-theme-bg2 rounded" /><div className="h-4 w-10 bg-theme-bg2 rounded" /></div>
                  </div>
                </div>
              ))
            ) : projects.length === 0 ? (
              <div className="col-span-full pt-8 text-theme-text2">No projects found. Check the Admin panel to add some.</div>
            ) : null}
            ...
            {projects.map((proj, i) => (
              ...
              <div className="h-40 border-b bg-theme-bg2 flex items-center justify-center text-4xl border-theme-border relative overflow-hidden shrink-0">
                  {proj.coverImage ? (
                  ...
                  {proj.status && (
                    <span className={`absolute top-2.5 right-2.5 font-mono text-[9px] px-2 py-[3px] rounded font-medium z-10 ${proj.status.toLowerCase().includes('live')
            ...
            <div className="w-full md:w-1/2">
              ...
              <div className="flex flex-col gap-8">
                {experienceItems.map((item: any, i: number) => (
                  <div key={`exp-${i}`} className="flex gap-4 group">
                    <div className="flex flex-col items-center">
                      <div className={`w-2.5 h-2.5 rounded-full ${i === 0 ? 'bg-theme-accent group-hover:scale-125' : 'bg-theme-bg2 border border-theme-border group-hover:border-theme-accent'} transition-all duration-300`}></div>
                      <div className="w-px h-full bg-theme-border mt-2"></div>
                    </div>
                    <div className="pb-2">
                      <div className={`font-mono text-[10px] mb-1 ${i === 0 ? 'text-theme-accent' : 'text-theme-text3'}`}>{item.year}</div>
                      <h4 className="font-serif text-lg text-theme-text">{item.role}</h4>
                      {item.company && <div className="text-sm text-theme-text2 mb-2">{item.company}</div>}
            ...
            <div className="w-full md:w-1/2">
              ...
              <div className="flex flex-col gap-8">
                {educationItems.map((item: any, i: number) => (
                  <div key={`edu-${i}`} className="flex gap-4 group">
                    <div className="flex flex-col items-center">
                      <div className={`w-2.5 h-2.5 rounded-full ${i === 0 ? 'bg-theme-accent group-hover:scale-125' : 'bg-theme-bg2 border border-theme-border group-hover:border-theme-accent'} transition-all duration-300`}></div>
                      <div className="w-px h-full bg-theme-border mt-2"></div>
                    </div>
                    <div className="pb-2">
                      <div className={`font-mono text-[10px] mb-1 ${i === 0 ? 'text-theme-accent' : 'text-theme-text3'}`}>{item.year}</div>
                      <h4 className="font-serif text-lg text-theme-text">{item.role}</h4>
                      {item.institution && <div className="text-sm text-theme-text2 mb-2">{item.institution}</div>}
                      <p className="text-xs text-theme-text3 leading-relaxed">
                        {item.desc}
                      </p>
                    </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {loadingBlogs ? (
              Array.from({ length: 2 }).map((_, i) => (
                <div key={i} className="bg-theme-card border border-theme-border rounded-3xl p-8 animate-pulse flex flex-col md:flex-row gap-6 items-center">
                  <div className="h-48 w-full md:w-1/2 bg-theme-bg2 rounded-lg" />
                  <div className="w-full md:w-1/2 space-y-3">
                    <div className="h-3 w-24 bg-theme-bg2 rounded" />
                    <div className="h-5 w-3/4 bg-theme-bg2 rounded" />
                    <div className="h-3 w-full bg-theme-bg2 rounded" />
                    <div className="h-3 w-2/3 bg-theme-bg2 rounded" />
                  </div>
                </div>
              ))
            ) : blogs.length === 0 ? (
              <div className="col-span-full pt-8 text-theme-text2">No blogs found. Check the Admin panel to add some.</div>
            ) : null}
